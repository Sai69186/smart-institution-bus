"""Benchmark A* Search on 1000-stop dataset."""
import sys, os, time, random, joblib
import pandas as pd
import numpy as np

sys.path.insert(0, os.path.dirname(__file__))
from route_optimizer import astar, haversine_km
...
for r in routes:
    ...
    result  = astar(r['stops'], r['si'], r['di'])
...
print(f"A*       | before={stats['avg_before_km']} km | after={stats['avg_after_km']} km | saved={stats['avg_saved_pct']}% | time={stats['avg_time_ms']} ms")
...
opt_stats['AStar'] = stats
joblib.dump(opt_stats, opt_path)
print('Saved AStar to optimization_stats.pkl')
