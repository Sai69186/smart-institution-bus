"""Benchmark Nearest-Neighbor + 2-opt on 1000-stop dataset."""
import sys, os, time, random, joblib
import pandas as pd
import numpy as np

sys.path.insert(0, os.path.dirname(__file__))
from route_optimizer import nearest_neighbor_2opt, haversine_km
...
for r in routes:
    ...
    result = nearest_neighbor_2opt(r['stops'], r['si'], r['di'])
...
print(f"NN+2opt  | before={stats['avg_before_km']} km | after={stats['avg_after_km']} km | saved={stats['avg_saved_pct']}% | time={stats['avg_time_ms']} ms")
...
opt_stats['NN_2opt'] = stats
joblib.dump(opt_stats, opt_path)
print('Saved NN_2opt to optimization_stats.pkl')
